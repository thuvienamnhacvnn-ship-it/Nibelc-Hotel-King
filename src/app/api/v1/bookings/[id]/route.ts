...
export const GET = api<{ id: string }>(async (_req, actor, { id }) => getBookingDetail(actor, id));

/** PATCH — chỉ sửa thông tin không ảnh hưởng tồn; bắt buộc expectedVersion (409 stale_version nếu cũ). */
export const PATCH = api<{ id: string }>(async (req, actor, { id }) => updateBookingDetails(actor, id, await readJson(req)));
